...
(function () {
  ...
  function actions(grade, subject, cur, cat) {
    ...
    return `<div class="doc-actions">
      ...
      ${gAtt ? `<a class="btn btn-outline btn-sm" href="${encodeURI(gAtt.file)}" download="${esc(gAtt.name)}" title="${esc(gAtt.name)}">${icon('download')} Cả khối ${grade} (Word)</a>` : ''}
      ${(CT.store.data.gradeAttachmentsXlsx || {})[String(grade)] ? `<a class="btn btn-outline btn-sm" href="${encodeURI(CT.store.data.gradeAttachmentsXlsx[String(grade)].file)}" download="${esc(CT.store.data.gradeAttachmentsXlsx[String(grade)].name)}">${icon('download')} Cả khối ${grade} (Excel)</a>` : ''}
